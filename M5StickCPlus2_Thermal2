...
static const uint32_t k_hotspot_click_window_ms = 1500;
...
static const char *WIFI_PASS = "wifi-pass";
...
static void playEasterEggChime() {
    ...
    M5.Speaker.setVolume(k_sound_volume_easter);
    for (size_t i = 0; i < sizeof(notes) / sizeof(notes[0]); ++i) {
        M5.Speaker.tone(notes[i], durations[i]);
        delay(durations[i] + 20);
    }
...
void loop(void) {
    ...
    if (g_screen_blank && (now - g_blank_last_frame_ms) < k_blank_frame_interval_ms) {
        allow_frame = false;
    }
    ...
    if (allow_frame && thermal2.update()) {
        ...
        if (!g_screen_blank) {
            ...
            info_canvas.setTextSize(2);
            int y1 = start_y + big_h + 2 + line_h / 2;
            info_canvas.setTextColor(WHITE);
            ...
            int y3 = y2 + line_h + 2;
            info_canvas.setTextColor(TFT_BLUE);
    ...
    if (!g_auto_poweroff_inhibit && !g_is_charging) {
        if (now_check - g_last_activity_ms >= k_auto_poweroff_ms) {
            playPoweroffChime();
            M5.Power.powerOff();
        }
    }
